...
from transformers.trainer import (
    ...
    TRAINER_STATE_NAME, DebugUnderflowOverflow,
    DistributedType, ParallelMode, SeedableRandomSampler,
    skip_first_batches, nested_detach, reissue_pt_warnings,
)
...
class LLaVATrainer(Trainer):
    def __init__(self, **kwargs):
        ...
        # Lazily initialized in compute_loss from tokenizer
        self._temporal_id_threshold = None   # ID of <time_start>
        self._bin_start_id          = None   # ID of <t000>

    # ─────────────────────────────────────────────────────────────────────
    # Composite temporal loss  L = Lce + λ·Lgrounding
    # ─────────────────────────────────────────────────────────────────────
    def compute_loss(self, model, inputs, return_outputs=False):
        """
        Composite loss implementing the proposal's Phase 2 objective:

            L = Lce + λ · Lgrounding

        Lce  — standard next-token CE, with optional temporal_loss_weight
               multiplier on <time_start>, <time_end>, <t000>–<t999> tokens
               to compensate for their small share (~13%) of per-sample loss.

        Lgrounding — distance-based penalty on bin-token predictions:
               For each position where the GT label is a bin token <tXXX>,
               we compute the expected bin index under the predicted distribution
               (softmax restricted to the 1000 bin tokens) and penalise its
               L1 distance from the GT bin, normalised to [0,1]:

                   Lgrounding = mean_i(|E[bin_i] - gt_bin_i| / (N-1))

               Unlike CE, this loss is sensitive to distance: predicting
               <t001> when GT is <t002> is penalised far less than predicting
               <t999>.  This breaks the degeneracy where the model defaults
               to <t000> as a "safe" CE-minimising choice.

        Falls back to standard HF Trainer compute_loss if both weights are
        at their default (1.0 / 0.0) values.
        """
        ce_weight      = getattr(self.args, "temporal_loss_weight", 1.0)
        lambda_ground  = getattr(self.args, "lambda_grounding",      1.0)
        num_bins       = getattr(self.args, "temporal_num_bins",      1000)

        trivial = (ce_weight == 1.0 and lambda_ground == 0.0)
        if trivial:
            return super().compute_loss(model, inputs, return_outputs=return_outputs)

        # ── lazily resolve temporal token ID boundaries ───────────────────
        if self._temporal_id_threshold is None:
            tok    = getattr(self, "tokenizer", None) or getattr(self, "processing_class", None)
            ts_id  = tok.convert_tokens_to_ids("<time_start>") if tok else None
            unk_id = getattr(tok, "unk_token_id", None)
            if ts_id is not None and ts_id != unk_id:
                self._temporal_id_threshold = ts_id
                self._bin_start_id          = ts_id + 2
                rank0_print(f"[temporal loss] <time_start>={ts_id}  <t000>={ts_id+2}  "
                            f"ce_weight={ce_weight}  λ_grounding={lambda_ground}")
            else:
                rank0_print("[temporal loss] WARNING: <time_start> not in tokenizer; "
                            "temporal weighting/grounding disabled.")
                self._temporal_id_threshold = 10**9
                self._bin_start_id          = 10**9

        # ── KEY FIX: get expanded labels ──────────────────────────────────
        # LLaVA replaces <image> tokens with visual patch features inside
        # forward(), stretching T from ~227 to ~2700.  If we grab
        # inputs["labels"] after the call, it's still the original short
        # tensor and won't match logits.  We call prepare_inputs_labels_for_multimodal
        # ourselves first so we hold onto the expanded labels, then pass
        # inputs_embeds to the model so it skips the internal prepare step.
        ...
        (
            _,          # input_ids (None after prepare)
            _pos,       # position_ids
            _attn,      # attention_mask (expanded)
            _,          # past_key_values
            inputs_embeds,  # (B, T_expanded, D)  — image features already merged
            labels_exp,     # (B, T_expanded)     — labels aligned with expanded seq
        ) = prepare_fn(
        ...
        # Forward with pre-computed embeds — no second prepare call inside model
        outputs = model(
            input_ids           = None,
            ...
            labels              = None,   # compute loss manually below
            use_cache           = False,
        ...
        # ── next-token shift — now shapes match ───────────────────────────
        shift_logits = logits[..., :-1, :].contiguous()      # (B, T-1, V)
        shift_labels = labels_exp[..., 1:].contiguous()      # (B, T-1)  ← expanded

        threshold    = self._temporal_id_threshold
        bin_start_id = self._bin_start_id

        # ─────────────────────────────────────────────────────────────────
        # Lce  (weighted cross-entropy)
        # ─────────────────────────────────────────────────────────────────
        ...
        tok_weights                     = torch.ones_like(per_tok)
        is_temporal                     = (shift_labels >= threshold) & (shift_labels != IGNORE_INDEX)
        tok_weights[is_temporal]        = ce_weight

        mask   = (shift_labels != IGNORE_INDEX).float()
        denom  = mask.sum().clamp(min=1.0)
        l_ce   = (per_tok * tok_weights * mask).sum() / denom

        # ─────────────────────────────────────────────────────────────────
        # Lgrounding  (expected-bin distance)
        #
        # At each position where GT label is a bin token <tXXX>:
        #   1. Slice logits for the 1000 bin tokens only.
        #   2. Softmax → probability distribution over bins.
        #   3. Expected bin = Σ_i  i · p_i
        #   4. |E[bin] - gt_bin| / (N-1)  ∈ [0, 1]
        # ─────────────────────────────────────────────────────────────────
        l_grounding = torch.tensor(0.0, device=l_ce.device, dtype=l_ce.dtype)
        if lambda_ground > 0.0 and bin_start_id < V:
            bin_end_id = bin_start_id + num_bins    # exclusive upper bound
            is_bin     = (shift_labels >= bin_start_id) & \
                         (shift_labels <  bin_end_id)   # (B, T-1) bool

            if is_bin.any():
                # Logits restricted to bin vocabulary  (B, T-1, num_bins)
                bin_logits = shift_logits[..., bin_start_id:bin_end_id]

                # Expected bin index  E[bin] = Σ_i i·p_i   (B, T-1)
                bin_probs   = torch.softmax(bin_logits, dim=-1)
                bin_idx_vec = torch.arange(num_bins,
                                           device=bin_logits.device,
                                           dtype=bin_logits.dtype)
                expected_bins = (bin_probs * bin_idx_vec).sum(dim=-1)   # (B, T-1)

                # GT bin indices  (offset label IDs → 0..N-1)
                gt_bins = (shift_labels - bin_start_id).float().clamp(0, num_bins - 1)

                # Normalised L1 distance
                dist = torch.abs(expected_bins - gt_bins) / (num_bins - 1)

                bin_mask    = is_bin.float()
                l_grounding = (dist * bin_mask).sum() / bin_mask.sum().clamp(min=1.0)

        loss = l_ce + lambda_ground * l_grounding
    ...
    def create_accelerator_and_postprocess(self):
        ...
        # accelerate >= 0.27 moved dispatch_batches/split_batches into DataLoaderConfiguration
