...
class ModelArguments:
    ...
    mm_use_temporal_ground_tokens: bool = field(
        default=False,
        metadata={"help": "Add <time_start>, <time_end>, and <t###> discrete time-bin tokens for temporal grounding."},
    )
    mm_num_time_bins: int = field(default=1000, metadata={"help": "Number of uniform time bins N (indices 0..N-1)."})
...
class DataArguments:
    ...
    eval_data_path: Optional[str] = field(default=None, metadata={"help": "Path to eval/validation data JSON (same format as data_path). If set, enables evaluation during training."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    temporal_loss_weight: float = field(
        default=1.0,
        metadata={"help": "CE loss multiplier for temporal bin tokens (<t000>–<t999>, <time_start>, <time_end>). "
                          "Values >1 up-weight temporal localization vs text answer quality."},
    )
    lambda_grounding: float = field(
        default=1.0,
        metadata={"help": "Weight λ for Lgrounding in composite loss L = Lce + λ·Lgrounding. "
                          "Lgrounding penalizes the expected-bin distance from GT bin (normalized to [0,1]). "
                          "Set to 0 to disable. Recommended range: 0.5–5.0."},
    )
    temporal_num_bins: int = field(
        default=1000,
        metadata={"help": "Number of temporal quantization bins N (must match mm_num_time_bins)."},
    )
    early_stopping_patience: int = field(
        default=0,
        metadata={"help": "Stop if eval_loss does not improve for this many evaluations. "
                          "0 = disabled. Requires evaluation_strategy != 'no' and load_best_model_at_end=True."},
    )
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    rank0_print(f"[llava-train] Train LazySupervisedDataset size = {len(train_dataset)} samples")
    eval_dataset = None
    if getattr(data_args, "eval_data_path", None):
        ...
        rank0_print(f"[llava-train] Eval  LazySupervisedDataset size = {len(eval_dataset)} samples")
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            # Pathnames like .../LLaVA_Model_Weights do not match the heuristics above; use config.architectures.
            _fp_kw = dict(
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            arch0 = None
            try:
                cfg_inf = AutoConfig.from_pretrained(model_args.model_name_or_path)
                arch_list = getattr(cfg_inf, "architectures", None) or []
                arch0 = arch_list[0] if arch_list else None
            except Exception:
                pass
            if arch0 == "LlavaLlamaForCausalLM":
                model = LlavaLlamaForCausalLM.from_pretrained(model_args.model_name_or_path, **_fp_kw)
            elif arch0 == "LlavaMistralForCausalLM":
                model = LlavaMistralForCausalLM.from_pretrained(model_args.model_name_or_path, **_fp_kw)
            elif arch0 == "LlavaMixtralForCausalLM":
                model = LlavaMixtralForCausalLM.from_pretrained(model_args.model_name_or_path, **_fp_kw)
                from transformers.models.mixtral.modeling_mixtral import MixtralSparseMoeBlock

                deepspeed.utils.set_z3_leaf_modules(model, [MixtralSparseMoeBlock])
            elif arch0 == "LlavaQwenForCausalLM":
                model = LlavaQwenForCausalLM.from_pretrained(model_args.model_name_or_path, **_fp_kw)
            elif arch0 == "LlavaQwenMoeForCausalLM":
                model = LlavaQwenMoeForCausalLM.from_pretrained(model_args.model_name_or_path, **_fp_kw)
                from transformers.models.qwen2_moe.modeling_qwen2_moe import Qwen2MoeSparseMoeBlock

                deepspeed.utils.set_z3_leaf_modules(model, [Qwen2MoeSparseMoeBlock])
            elif arch0 == "LlavaGemmaForCausalLM":
                model = LlavaGemmaForCausalLM.from_pretrained(model_args.model_name_or_path, **_fp_kw)
            else:
                raise ValueError(
                    f"Unknown model class {model_args} "
                    f"(path substring heuristics failed; config architectures={getattr(cfg_inf, 'architectures', None)!r}). "
                    f"Pass --model_class_name LlavaLlama (or rename the checkpoint folder to include llama/vicuna/qwen)."
                )
    else:
...
def train(attn_implementation=None):
    ...
    rank0_print(
        "[llava-train] Parsed args | model_name_or_path=",
        model_args.model_name_or_path,
        "| data_path=",
        data_args.data_path,
        "| output_dir=",
    ...
    rank0_print("[llava-train] Loading base model (get_model / from_pretrained) …")
    model = get_model(model_args, training_args, bnb_model_from_pretrained_args)
    rank0_print("[llava-train] Base model loaded.")
    ...
    rank0_print("[llava-train] Loading tokenizer …")
    if "mistral" in model_args.model_name_or_path.lower() or "mixtral" in model_args.model_name_or_path.lower() or "zephyr" in model_args.model_name_or_path.lower():
        ...
        # Folder names like LLaVA_Model_Weights do not match the path heuristics above.
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir,
            model_max_length=training_args.model_max_length,
    ...
    rank0_print("[llava-train] Tokenizer ready.")
    ...
    if model_args.vision_tower is not None:
        rank0_print("[llava-train] Initializing vision modules (vision tower / projector) …")
        model.get_model().initialize_vision_modules(model_args=model_args, fsdp=training_args.fsdp)

        vision_tower = model.get_vision_tower()
        rank0_print("[llava-train] Moving vision tower to device …")
        ...
        rank0_print("[llava-train] Initializing vision tokenizer (image patch / temporal tokens) …")
        model.initialize_vision_tokenizer(model_args, tokenizer=tokenizer)
        rank0_print("[llava-train] Vision tokenizer done.")

    # ── LoRA 必须在 initialize_vision_tokenizer（resize_token_embeddings）之后应用，
    # ── 否则 ModulesToSaveWrapper 会导致 resize 时找不到 .weight ──────────────────
    if training_args.lora_enable:
        ...
        if getattr(model_args, "mm_use_temporal_ground_tokens", False):
            lora_kw["modules_to_save"] = ["model.embed_tokens", "lm_head"]
            rank0_print(f"LoRA modules_to_save (temporal tokens): {lora_kw['modules_to_save']}")
        ...
        rank0_print("[llava-train] LoRA adapters attached.")

    # gradient checkpointing 也在 LoRA 之后设置，钩子才会挂在正确的模块上
    ...
    rank0_print("[llava-train] Building dataset and collator (lazy load: first __getitem__ may be slow on Drive) …")
    data_module = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args)
    rank0_print("[llava-train] Dataset object ready; constructing trainer …")
    _callbacks = []
    _patience = getattr(training_args, "early_stopping_patience", 0)
    if _patience > 0:
        from transformers import EarlyStoppingCallback
        _callbacks.append(EarlyStoppingCallback(early_stopping_patience=_patience))
        rank0_print(f"[llava-train] EarlyStoppingCallback enabled (patience={_patience})")
    trainer = LLaVATrainer(
        model=model, tokenizer=tokenizer, args=training_args,
        callbacks=_callbacks if _callbacks else None,
        **data_module,
    )
    rank0_print("[llava-train] Starting trainer.train() …")
