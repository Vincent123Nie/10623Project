...
from llava.temporal_tokens import add_temporal_ground_tokens_to_model
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="sdpa", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if load_8bit:
        ...
        kwargs["quantization_config"] = BitsAndBytesConfig(load_in_8bit=True, bnb_4bit_compute_dtype=torch.float16, bnb_4bit_use_double_quant=True, bnb_4bit_quant_type="nf4")
    elif load_4bit:
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if os.path.exists(os.path.join(model_path, "non_lora_trainables.bin")):
                ...
                # Local checkpoint with no non_lora_trainables.bin — weights are in
                # adapter_model.safetensors via PEFT modules_to_save; nothing to load here.
                non_lora_trainables = {}
            else:
            ...
            # If the checkpoint saved extra tokens (e.g. temporal tokens via
            # modules_to_save), its embed_tokens will be larger than the base model.
            # Resize before loading the PEFT adapter to avoid size mismatch.
            _ckpt_tok_cfg = os.path.join(model_path, "tokenizer_config.json")
            if os.path.exists(_ckpt_tok_cfg):
                _ckpt_tok = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                _cur_size = model.get_input_embeddings().weight.shape[0]
                if len(_ckpt_tok) != _cur_size:
                    rank0_print(f"Resizing embeddings {_cur_size} → {len(_ckpt_tok)} before loading LoRA adapter...")
                    tokenizer = _ckpt_tok
                    _in_dev  = model.get_input_embeddings().weight.device
                    _out_dev = model.get_output_embeddings().weight.device
                    model.get_input_embeddings().to("cpu")
                    model.get_output_embeddings().to("cpu")
                    model.resize_token_embeddings(len(tokenizer))
                    model.get_input_embeddings().to(_in_dev)
                    model.get_output_embeddings().to(_out_dev)
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if getattr(model.config, "mm_use_temporal_ground_tokens", False):
            add_temporal_ground_tokens_to_model(
                tokenizer,
                model,
                num_bins=int(getattr(model.config, "mm_num_time_bins", 1000)),
            )
