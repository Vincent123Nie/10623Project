    ...
    try:
        ...
        def apply_chunking_to_forward(
            ...
            *input_tensors: torch.Tensor,
            ...
            """Fallback when transformers drops apply_chunking_to_forward (e.g. Colab preinstall)."""
            ...
            for t in input_tensors:
                if t.shape[chunk_dim] != tensor_shape:
                    raise ValueError(
                        f"Chunk dim {chunk_dim}: expected length {tensor_shape}, got {t.shape[chunk_dim]}"
                    )
            num_chunks = tensor_shape // chunk_size + (1 if tensor_shape % chunk_size else 0)
            if num_chunks <= 1:
                return forward_fn(*input_tensors)
            chunks = [x.chunk(num_chunks, dim=chunk_dim) for x in input_tensors]
            outs = [forward_fn(*[c[i] for c in chunks]) for i in range(num_chunks)]
